...
async function fetchMemberProfiles({ forceRefresh = false } = {}) {
  if (!forceRefresh && Date.now() - memberCache.fetchedAt < CACHE_TTL_MS && memberCache.profiles.length) {
...
async function fetchBadgeRules({ forceRefresh = false } = {}) {
  if (!forceRefresh && Date.now() - badgeRuleCache.fetchedAt < CACHE_TTL_MS && badgeRuleCache.rules.length) {
...
async function findMemberProfileByUsername(username, options = {}) {
  const target = normalizeUsername(username);
  if (!target) return null;
  const profiles = await fetchMemberProfiles(options);
...
async function getPublicMemberProfileByUsername(username, { forceRefresh = false } = {}) {
  const [profile, badgeRules] = await Promise.all([
    findMemberProfileByUsername(username, { forceRefresh }),
    fetchBadgeRules({ forceRefresh }),
  ]);
